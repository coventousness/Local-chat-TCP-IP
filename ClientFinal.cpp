...
int CallbackLastMessage(void* context, int columnCount, char** columnValues, char** columnNames)
{
	std::string z = columnValues[0];
	z += ":";
	z += columnValues[1];
	base.emplace_back(z);
	return 0;
...
int ClientHandler() 
{
	...
	while (true)
	{
		error = recv(Connection, (char*)&id, sizeof(id), 0);
		error = recv(Connection, name, sizeof(name), 0);
		error = recv(Connection, message, sizeof(message), 0);
		if (error <= 0) { std::cout << "Error: failed connect to server."; break; }
		...
		Table_Message = "INSERT INTO NAMES VALUES(" + std::to_string(id) + ",\"" + name + "\");";
		if (sqlite3_exec(db, Table_Message.c_str(), 0, 0, &err))
		{
			//std::cout << "Помилка SQL Table_Message INSERT: " << err;
			sqlite3_free(err);
...
int registration()
{
	...
	else
	{
		...
		recv(Connection, (char*)&id, sizeof(id), NULL);
		std::system("cls");
		if (id == 0) { std::cout << "FailLogin OR FailPassword\n"; goto Fail; }
		std::cout << "\nYou id: " << id;
	}
	return id;
}
...
int main()
{
	...
	std::cout << "Connected!\n";
	char* err = 0;
	if (sqlite3_open("database_client", &db))
	{
	...
	for (;;)
	{
		...
		Table_Message = "SELECT NAMES.NAME,MESSAGE FROM NAMES,ID" + std::to_string(IDfrendly) + " WHERE(ID" + std::to_string(IDfrendly) + ".SENDER == NAMES.ID) ORDER BY ID" + std::to_string(IDfrendly) + ".ID DESC LIMIT 100; ";
		if (sqlite3_exec(db, Table_Message.c_str(), CallbackLastMessage, 0, &err))
		{
			std::cout << "Помилка SQL CallbackLastMessage: " << err;
			sqlite3_free(err);
		}

		for (int i = 0; i < base.size(); ++i)
		{
			std::cout << base[base.size() - 1 - i] << std::endl;
		}
		for (;;)
		{
			...
			error = send(Connection, (char*)&IDfrendly, sizeof(IDfrendly), 0);
			error = send(Connection, name_char, sizeof(name_char), 0);
			error = send(Connection, message, sizeof(message), 0);
			if (error <= 0) {
				std::cout << "Error: failed connect to server.\n";
				Sleep(1000);
			}
			...
			Table_Message = "INSERT INTO ID" + std::to_string(IDfrendly) + " VALUES(" + std::to_string(Handler::id) +","+ std::to_string(MyID)+",\"" + message + "\");";
			if (sqlite3_exec(db, Table_Message.c_str(), 0, 0, &err))
			{
				std::cout << "Помилка SQL Table_Message INSERT: " << err;
				sqlite3_free(err);
